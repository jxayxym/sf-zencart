	...
	switch($action) {
		case 'enable-parent':
			if(SEO_URL_CATEGORY_DIR == 'full') {
				...
				echo '<div><span class="alert">' . sprintf(SEO_CONFIG_ADJUSTED, SEO_URL_CATEGORY_DIR, 'short') . '</span></div>';
			}
	...
	switch($action) {
		...
		case 'enable-full':
			if(SEO_URL_FORMAT == 'parent') {
				...
				echo '<div><span class="alert">' . sprintf(SEO_CONFIG_ADJUSTED, SEO_URL_FORMAT_TITLE, 'original') . '</span></div>';
			}
		case 'enable-short':
			$action = substr($action, 7);
			zen_db_perform(TABLE_CONFIGURATION, array('configuration_value' => $action), 'update', '`configuration_key`=\'SEO_URL_CATEGORY_DIR\'');
...
// Function to chech the global cache settings
function usu_check_cache_options($action) {
	$temp = explode('-', $action);
	if(sizeof($temp) < 2) $temp[] = 'general';
	$temp[1] = strtoupper($temp[1]);

	switch($temp[0]) {
		case 'enable':
			$action = 'true';
			if(SEO_USE_CACHE_GLOBAL == 'false' && $temp[1] != 'GENERAL') {
				zen_db_perform(TABLE_CONFIGURATION, array('configuration_value' => $action), 'update', '`configuration_key`=\'SEO_USE_CACHE_GLOBAL\'');
				echo '<div><span class="alert">' . sprintf(SEO_CONFIG_ADJUSTED, SEO_USE_CACHE_GLOBAL, $action) . '</span></div>';
			}
		case 'disable':
			$action = 'false';
