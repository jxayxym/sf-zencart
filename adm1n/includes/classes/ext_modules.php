 ...
 * @version $Id: ext_modules.php, v 1.3 07.08.2011 19:54:52 AndrewBerezin $
 */

class ext_modules {
  public $configuration_group_id = null;
  public $prefix = null;
  public $configUpdates = array();
  ...
  //      echo '<pre>'.__LINE__.': : ';var_export($tableFieldsNew);echo '</pre>';
    //    die('Изменилась структура таблицы ' . $table);
        ...
        foreach ($tableFieldsNew as $field) {
          ...
          if (!isset($tableFieldsOld[$field])) {
            $this->_add_field($table, $field);
          } else {
            unset($tableFieldsOld[$field]);
          }
        }
  //      echo '<pre>'.__LINE__.': : ';var_export($tableFieldsOld);echo '</pre>';
        foreach ($tableFieldsOld as $field) {
          list($field, $field_parms) = $this->_extract_field($field);
          $db->Execute("ALTER TABLE `" . $table . "` DROP `" . zen_db_input($field) . "`;");
        }
      }
    }
