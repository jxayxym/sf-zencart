...
<?php
    for ($i=0, $n=sizeof($order->products); $i<$n; $i++) {
      ...
      echo '          <tr class="dataTableRow">' . "\n" .
           ...
           '            <td class="dataTableContent" valign="top">(#' .$order->products[$i]['id'].')' .$order->products[$i]['name'];
...
?>
                ...
                <td class="dataTableContent"><?php echo '<a href="' . zen_href_link(FILENAME_CUSTOMERS, 'cID=' . $orders->fields['customers_id'], 'NONSSL') . '">' . zen_image(DIR_WS_ICONS . 'preview.gif', ICON_PREVIEW . ' ' . TABLE_HEADING_CUSTOMERS) . '</a>&nbsp;' . (zen_not_null($orders->fields['customers_name'])?$orders->fields['customers_name']:'#cID-'.$orders->fields['customers_id']) . ($orders->fields['customers_company'] != '' ? '<br />' . $orders->fields['customers_company'] : ''); ?></td>
