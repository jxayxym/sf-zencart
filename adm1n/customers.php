...
//    zen_redirect(zen_href_link(FILENAME_CUSTOMERS, 'cID=' . $_GET['cID'] . (isset($_GET['page']) ? '&page=' . $_GET['page'] : ''), 'NONSSL'));
    ...
    while (!$customers->EOF) {
      ...
      if ((!isset($_GET['cID']) || (isset($_GET['cID']) && ($_GET['cID'] == $customers->fields['customers_id']))) && !isset($cInfo)) {
		...
		if (!is_array($info->fields)) $info->fields = array();
  ...
  switch ($action) {
    ...
    default:
      ...
      if (isset($cInfo) && is_object($cInfo)) {
        ...
        $heading[] = array('text' => '<b>' . TABLE_HEADING_ID . $cInfo->customers_id . ' ' . $cInfo->customers_firstname . ' ' . $cInfo->customers_lastname . '<span style="margin-left:10px;">'.$cInfo->customers_email_address.'</span></b>');
