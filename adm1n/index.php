...
<div class="header"><?php echo BOX_ENTRY_NEW_CUSTOMERS; ?> </div>
  ...
  while (!$customers->EOF) {
    ...
    echo '              <div class="row"><span class="left"><a href="' . zen_href_link(FILENAME_CUSTOMERS, 'search=' . $customers->fields['customers_email_address'] . '&origin=' . FILENAME_DEFAULT, 'NONSSL') . '" class="contentlink">#'.$customers->fields['customers_id'].' '.$customers->fields['customers_firstname'] . ' ' . $customers->fields['customers_lastname'] .((empty($customers->fields['customers_firstname'])&&empty($customers->fields['customers_lastname']))?$customers->fields['customers_email_address']:'').'</a></span><span class="rigth">' . "\n";
...
<div class="header"><?php echo BOX_ENTRY_NEW_ORDERS; ?> </div>
  ...
  while (!$orders->EOF) {
    echo '              <div class="row"><div><span><a href="' . zen_href_link(FILENAME_ORDERS, 'oID=' . $orders->fields['orders_id'] . '&origin=' . FILENAME_DEFAULT, 'NONSSL') . '" class="contentlink"> ' . (zen_not_null($orders->fields['customers_name'])?$orders->fields['customers_name']:'Customer ID:#'.$orders->fields['customers_id']) . '</a></span><span class="right">' . $orders->fields['order_total'] . '</span></div>' . "\n";
    echo '<div style="clear:both;"><span>Order ID:#'.$orders->fields['orders_id'].'</span><span class="right">'.zen_date_short($orders->fields['date_purchased']).'</span></div>';
    echo '              </div>' . "\n";
    $orders->MoveNext();
  }
?>
