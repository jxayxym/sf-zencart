 ...
 * @version $Id: sitemapxml.php, v 3.2.5 31.05.2012 14:52:55 13:38:59 AndrewBerezin $
 */
    ...
    if ($this->genxml) {
      ...
      foreach ($sitemapFiles as $filename) {
        ...
        if ($filenameBase != $this->sitemapindex && filesize($filename) > 0) {
          ...
          echo TEXT_INCLUDE_FILE . $filename . ' (<a href="' . $fileURL . '" target="_blank">' . $fileURL . '</a>)' . '<br />';
          $itemRecord = '';
          $itemRecord .= ' <sitemap>' . "\n";
...
// retrieve full cPath from category ID
  ...
  function imagesTags($images, $caption='true', $license_url='') {
    ...
    foreach ($images as $image) {
      ...
      $tags .= '  <image:image>' . "\n";
      $tags .= '    <image:loc>' . $this->_clear_url($loc) . '</image:loc>' . "\n";
      if ($caption == 'true') {
      ...
      if ($license_url != '') {
        ...
        $tags .= '    <image:license>' . $this->_clear_url($license_url) . '</image:license>' . "\n";
      }
      $tags .= '  </image:image>' . "\n";
    }
    return $tags;
