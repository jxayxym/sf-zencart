...
//    $current_category_id = $cPath;
	...
	if (empty($categories)){
		if ($master_categories_id = get_master_categories_id((int)$_GET['products_id'])) {
			$categories[] = get_master_categories_id((int)$_GET['products_id']);
		}
	}

if (zen_not_null($categories)) {
  $sql = "select p.products_id, p.products_model, p.products_price_sorter, pd.products_name, p.products_sort_order
          from   " . TABLE_PRODUCTS . " p, "
  . TABLE_PRODUCTS_DESCRIPTION . " pd, "
  . TABLE_PRODUCTS_TO_CATEGORIES . " ptc
          where  p.products_status = '1' and p.products_id = pd.products_id and pd.language_id= '" . (int)$_SESSION['languages_id'] . "' and p.products_id = ptc.products_id and ptc.categories_id in(" . implode(',', $categories) . ")" .
  $prev_next_order;
