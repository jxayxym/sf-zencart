...
//   echo $listing_split->sql_query;exit;
  ...
  while (!$listing->EOF) {
    ...
    'text' => '<div class="photo_frame">'.(($listing->fields['products_image'] == '' and PRODUCTS_IMAGE_NO_IMAGE_STATUS == 0) ? '' : '<a href="' . zen_href_link(zen_get_info_page($listing->fields['products_id']), 'cPath=' . $productsInCategory[$listing->fields['products_id']] . '&products_id=' . $listing->fields['products_id']) . '" class="listing_images_wrapper" >' . zen_image(DIR_WS_IMAGES . $listing->fields['products_image'], $listing->fields['products_name'], IMAGE_PRODUCT_LISTING_WIDTH, IMAGE_PRODUCT_LISTING_HEIGHT) .'</a><br />') . '<a href="' . zen_href_link(zen_get_info_page($listing->fields['products_id']), 'cPath=' . $productsInCategory[$listing->fields['products_id']] . '&products_id=' . $listing->fields['products_id']) . '" title="'.zen_output_string($listing->fields['products_name']).'">' . zen_trunc_string($listing->fields['products_name'],50) . '</a><div class="product_listing_price"><span class="product_listing_currency">' .$_SESSION['currency'] .' </span>'.$products_price.'</div><div class="product_listing_reviews">'.$products_reivews.'</div></div>');
