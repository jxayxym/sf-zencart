...
if ($r->RecordCount()>0){
	...
	'where po.products_options_type=5 group by pa.options_values_id';
...
// 					'link'=>zen_href_link(FILENAME_DEFAULT,$link_params.'options_values_id='.implode('_', $options_values_id_link)),
		...
		$title = BOX_HEADING_SHOP_BY;
		$box_id = 'box_shop_by';
		if (isset($_GET['options_values_id'])){
			$title .= '<a class="shopByClearAll" href="'.zen_href_link(FILENAME_DEFAULT,zen_get_all_get_params(array('options_values_id','sort'))).'"></a>';
		}
		$link = false;
