...
if ($num_products_count > 0) {
  ...
  while (!$new_products->EOF) {
    ...
    'text' => (($new_products->fields['products_image'] == '' and PRODUCTS_IMAGE_NO_IMAGE_STATUS == 0) ? '' : '<a href="' . zen_href_link(zen_get_info_page($new_products->fields['products_id']), 'cPath=' . $productsInCategory[$new_products->fields['products_id']] . '&products_id=' . $new_products->fields['products_id']) . '"><img src="'.DIR_WS_IMAGES . $new_products->fields['products_image'].'" alt="'.$new_products->fields['products_name'].'" width="'.IMAGE_PRODUCT_NEW_WIDTH.'px" height="'.IMAGE_PRODUCT_NEW_HEIGHT.'px" /></a><br />') . '<a href="' . zen_href_link(zen_get_info_page($new_products->fields['products_id']), 'cPath=' . $productsInCategory[$new_products->fields['products_id']] . '&products_id=' . $new_products->fields['products_id']) . '">' . zen_trunc_string($new_products->fields['products_name'],40) . '</a><p class="product_listing_price"><span class="product_listing_currency">'.$_SESSION['currency'].'</span>' . $products_price.'</p>');
	$list_box_contents[$row][$col]['text'] = '<div class="new_products_item">'.$list_box_contents[$row][$col]['text'].'<div class="new_product_flag"></div></div>'; 
    $col ++;
