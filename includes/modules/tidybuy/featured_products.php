...
if ($num_products_count > 0) {
  ...
  while (!$featured_products->EOF) {
    ...
    'text' => (($featured_products->fields['products_image'] == '' and PRODUCTS_IMAGE_NO_IMAGE_STATUS == 0) ? '' : '<a href="' . zen_href_link(zen_get_info_page($featured_products->fields['products_id']), 'cPath=' . $productsInCategory[$featured_products->fields['products_id']] . '&products_id=' . $featured_products->fields['products_id']) . '"><img src="'.DIR_WS_IMAGES . $featured_products->fields['products_image'].'" alt="'.$featured_products->fields['products_name'].'" width="'.IMAGE_PRODUCT_NEW_WIDTH.'px" height="'.IMAGE_PRODUCT_NEW_HEIGHT.'px" /></a><br />') . '<a href="' . zen_href_link(zen_get_info_page($featured_products->fields['products_id']), 'cPath=' . $productsInCategory[$featured_products->fields['products_id']] . '&products_id=' . $featured_products->fields['products_id']) . '">' . zen_trunc_string($featured_products->fields['products_name'],30) . '</a><br /><p class="product_listing_price"><span class="product_listing_currency">'.$_SESSION['currency'].'</span>' . $products_price);
