...
if ($num_products_count > 0) {
  ...
  while (!$specials_index->EOF) {
    ...
    'text' => (($specials_index->fields['products_image'] == '' and PRODUCTS_IMAGE_NO_IMAGE_STATUS == 0) ? '' : '<div class="t-a_c"><a href="' . zen_href_link(zen_get_info_page($specials_index->fields['products_id']), 'cPath=' . $productsInCategory[$specials_index->fields['products_id']] . '&products_id=' . (int)$specials_index->fields['products_id']) . '">' . zen_image(DIR_WS_IMAGES . $specials_index->fields['products_image'], $specials_index->fields['products_name'], IMAGE_PRODUCT_SPECIAL_WIDTH, IMAGE_PRODUCT_SPECIAL_HEIGHT,'style="border:1px solid #900;"') . '</a></div>') . '<div><a href="' . zen_href_link(zen_get_info_page($specials_index->fields['products_id']), 'cPath=' . $productsInCategory[$specials_index->fields['products_id']] . '&products_id=' . $specials_index->fields['products_id']) . '" title="'.$specials_index->fields['products_name'].'">' . zen_trunc_string($specials_index->fields['products_name'],25) . '</a></div>' . $products_price);
  ...
  if ($specials_index->RecordCount() > 0) {
    $title = '<div class="boxHeading"><h2>' . sprintf(TABLE_HEADING_SPECIALS_INDEX, strftime('%B')) . '</h2></div>';
