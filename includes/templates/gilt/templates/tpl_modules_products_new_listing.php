  ...
  if ($products_new_split->number_of_rows > 0) {
    ...
    while (!$products_new->EOF) {
      ...
      if (PRODUCT_NEW_LIST_NAME != '0') {
        $display_products_name = '<a href="' . zen_href_link(zen_get_info_page($products_new->fields['products_id']), 'cPath=' . zen_get_generated_category_path_rev($products_new->fields['master_categories_id']) . '&products_id=' . $products_new->fields['products_id']) . '"><strong>' . zen_trunc_string($products_new->fields['products_name'],60) . '</strong></a>' . str_repeat('<br clear="all" />', substr(PRODUCT_NEW_LIST_NAME, 3, 1));
      } else {
...
//            $how_many++;
      ...
      $reviews_info = array();	
      if (has_review($products_new->fields['products_id'],$reviews_info)){
